...
<template>
    <div class="flex relative items-center h-full">
        <div
            :class="`${has_won ? 'bg-red-3' : 'bg-blue-3'} flex items-center justify-end text-right w-full pr-3 ${height}`">
            <strong>
                {{ other_team_stat }}
            </strong>
        </div>
        <div
            :class="`${has_won ? 'bg-blue-3' : 'bg-red-3'} absolute text-left left-0 pl-3 flex items-center justify-start ${height}`"
